...
public class FilterManager {
    ...
    public void loadFilters() {
        ...
        if (Files.exists(filtersFile)) {
            try {
                ...
                Map<String, FilterDefinition> fileFilters = GSON.fromJson(json, mapType);
                if (fileFilters != null) {
                    loadedFilters.putAll(fileFilters);
                }
                LOGGER.info("Loaded " + loadedFilters.size() + " filters from " + filtersFile);
            } catch (Exception e) {
                LOGGER.severe("Failed to load filters from " + filtersFile + ": " + e.getMessage() + ". Attempting to use default filters.");
            }
        } else {
            LOGGER.info("Filters file not found at " + filtersFile + ". Attempting to create default filters.");
        }

        // Always load default filters and merge them with loaded filters
        Map<String, FilterDefinition> newFilters = new HashMap<>();

        // Add default filters first
        newFilters.putAll(getDefaultFilters());

        // Override with filters loaded from file (custom filters take precedence)
        newFilters.putAll(loadedFilters);

        // Update the instance's filters map
        this.filters = newFilters;

        // If no filters exist after loading and merging, create defaults and save
        if (this.filters.isEmpty()) {
            createDefaultFilters(); // This will also save them
        } else {
            saveFilters(); // Save the merged filters to ensure consistency
        }
    }
    ...
    private Map<String, FilterDefinition> getDefaultFilters() {
        ...
        defaultFilters.put("TRUMP_STYLE", new FilterDefinition("TRUMP_STYLE",
                ...
                "🇺🇸", "GOLD", true));
        defaultFilters.put("OPPOSITE", new FilterDefinition("OPPOSITE", "Rewrite this to mean the exact opposite while keeping it natural", "🔄", "AQUA", true));
        ...
        defaultFilters.put("CORPORATE_SPEAK", new FilterDefinition("CORPORATE_SPEAK",
                ...
                true));
        defaultFilters.put("PIRATE", new FilterDefinition("PIRATE", "Rewrite this as if spoken by an enthusiastic pirate, with 'arrr' and nautical terms",
                "🏴‍☠️", "GOLD", true));
        defaultFilters.put("SHAKESPEAREAN", new FilterDefinition("SHAKESPEAREAN", "Rewrite this in elaborate Shakespearean English with flowery language", "🎭",
                "DARK_PURPLE", true));
        defaultFilters.put("OVERLY_EXCITED", new FilterDefinition("OVERLY_EXCITED", "Rewrite this with MAXIMUM ENTHUSIASM!!! Use lots of exclamation points and caps",
                "🎉", "YELLOW", true));
        ...
        defaultFilters.put("ROBOT", new FilterDefinition("ROBOT",
                ...
                "🤖", "DARK_GRAY", true));
        defaultFilters.put("VALLEY_GIRL", new FilterDefinition("VALLEY_GIRL", "Rewrite this in valley girl speak with lots of 'like' and 'totally'", "💅",
                "LIGHT_PURPLE", true));
        defaultFilters.put("NOIR_DETECTIVE", new FilterDefinition("NOIR_DETECTIVE", "Rewrite this as if spoken by a 1940s film noir detective, dark and mysterious",
                "🕵️", "DARK_RED", true));
        ...
        defaultFilters.put("PASSIVE_AGGRESSIVE", new FilterDefinition("PASSIVE_AGGRESSIVE",
                ...
                "DARK_GREEN", true));
        defaultFilters.put("OVERSHARING", new FilterDefinition("OVERSHARING", "Rewrite this but add way too much personal information that nobody asked for", "📢",
                "LIGHT_PURPLE", true));
        defaultFilters.put("CONSPIRACY_FLAT_EARTH", new FilterDefinition("CONSPIRACY_FLAT_EARTH",
        ...
        defaultFilters.put("INFLUENCER", new FilterDefinition("INFLUENCER",
                ...
                "GOLD", true));
        defaultFilters.put("CAVEMAN", new FilterDefinition("CAVEMAN", "Rewrite this in simple caveman speak with basic words and concepts", "🦴", "DARK_AQUA",
                true));
